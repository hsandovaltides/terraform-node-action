...
async function run () {
  ...
  const validVersion = semver.validRange(version, { includePrerelease: false, loose: true }); // "latest" will return invalid but that's ok because we'll select it by default
	core.info(release);
  ...
  if (!validVersion) { // pick the latest release (prereleases will be skipped for safety, set an explicit version instead)
		...
		release = response.versions[version];
	} else {
